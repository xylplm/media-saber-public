...
Chrome 与 Edge 各提供三种格式；Firefox 与 Safari 各提供一种格式。普通用户优先使用扩展商店版本，只有无法访问商店或有特殊需求时才使用本页手动安装包。

| 文件 | 适用浏览器 | Manifest 版本 | 说明 |
|---|---|:---:|---|
| `media-saber-tools-chrome.zip` | Chrome / Chromium 内核浏览器 | MV3 | 解压后通过开发者模式加载 |
| `media-saber-tools-chrome.crx` | Chrome / Chromium 内核浏览器 | MV3 | CRX 打包文件，部分浏览器可直接拖入扩展管理页 |
| `media-saber-tools-chrome.crx.zip` | Chrome / Chromium 内核浏览器 | MV3 | 内含 CRX 文件，适合浏览器拦截 CRX 直链下载时使用 |
